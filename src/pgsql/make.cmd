...
:: Project URL ..: https://github.com/ki7mt/python-pypi-packages/jtenv
::
...
:: Make File Variables
SET app_name=ardslotw
SET pypitest=--repository-url https://test.pypi.org/legacy/ dist/*
SET test_install=-i https://test.pypi.org/simple/ jtenv
SET prod_install=-i https://pypi.org/simple/ jtenv
...
ECHO ----------------------------------------
ECHO.
IF EXIST %CD%\%requirements% (
    ECHO Installing Requirements using pip
    pip install ^-r %CD%\%requirements%
) else (
...
ECHO ----------------------------------------
ECHO.
IF EXIST %CD%\%requirements_dev% (
    ECHO Installing Developer Requirements using pip
    pip install ^-r %CD%\%requirements_dev%
) ELSE (
